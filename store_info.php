...
<body>
  ...
  $kbn = htmlspecialchars($_POST["vote"], ENT_QUOTES, "UTF-8");
    switch ($kbn) {
        case "営業中":
        $stmt7 = $pdo->query('SELECT o0_2 FROM sample0802_open WHERE store_id = '.$store_id);
        $result7 = $stmt7 -> fetch(PDO::FETCH_ASSOC);
        $result7['o0_2'] += 1;
        $stmt2 = $pdo->query('UPDATE sample0802_open SET o0_2='.$result['o0_2'].' WHERE store_id = '.$store_id);
        $result2 = $stmt2 -> fetch(PDO::FETCH_ASSOC); break;
